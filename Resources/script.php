...
$trello_member_id      = false;
$data				 = explode( ";", $argv[1] );
$trello_member_token = $data[0];
$trello_board_id     = $data[1];
$list_name		     = $data[2];
$member_username = $data[3];
$name 				 = (isset($data[4])) ? stripslashes(trim($data[4])) : 'Untitled card';
$desc 				 = (isset($data[5])) ? stripslashes(trim($data[5])) : '';
$url				 = "{$trello_api_endpoint}/boards/{$trello_board_id}?members=all&member_fields=username&lists=open&list_fields=name&fields=name,desc&key={$trello_key}&token={$trello_member_token}";
...
foreach($members as $member) {
	if($member->username == 'anderslemke') {
